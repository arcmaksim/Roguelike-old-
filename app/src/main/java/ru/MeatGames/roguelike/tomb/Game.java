...
public class Game extends Activity{
	...
	/**
	 * @uml.property  name="maxTiles"
	 */
	public final int maxTiles = 100;
	/**
	 * @uml.property  name="maxItems"
	 */
	public final int maxItems = 20;
	/**
	 * @uml.property  name="maxStats"
	 */
	public final int maxStats = 35;
	/**
	 * @uml.property  name="maxMobs"
	 */
    ...
    public void addInQueue(MobList mob){
        mob.turnCount = turnCount + mob.mob.spd;
        if(firstMob == null){
            firstMob = mob;
        }else{
            MobList cur;
	...
	public void attack(MapClass map){
		...
		if(att >= map.mob.mob.def){
			int u = rnd.nextInt(Global.hero.getStat(13)-Global.hero.getStat(12)+1)+Global.hero.getStat(12)-map.mob.mob.arm;
			if(u<1){
				u = 1;
			}
            map.mob.mob.hp -= u;
            Global.mapview.addLine(map.mob.mob.name+" �������� ����");
			if(map.mob.mob.hp<1){
				Global.mapview.addLine(map.mob.mob.name+" �������");
	...
	public void mobAttack(MobList mob){
		int att = rnd.nextInt(20)+1+mob.mob.att;
		if(att >= Global.hero.getStat(19)){
			int u = mob.mob.dmg - Global.hero.getStat(22);
			if(u<1){
				u = 1;
			}
			Global.hero.modifyStat(5,u,-1);
            Global.mapview.addLine(mob.mob.name+" ������� ����");
		}else{
			Global.mapview.addLine(mob.mob.name+" �������������");
		}
		if(Global.hero.getStat(5)<1){
        ...
        try{
            ...
            while(parser.getEventType()!=XmlPullParser.END_DOCUMENT){
                if(parser.getEventType() == XmlPullParser.START_TAG && parser.getName().equals("mob")){
                    Global.mobDB[i].mob.name = parser.getAttributeValue(0);
                    Global.mobDB[i].mob.hp = Integer.parseInt(parser.getAttributeValue(1));
                    Global.mobDB[i].mob.att = Integer.parseInt(parser.getAttributeValue(2));
                    Global.mobDB[i].mob.def = Integer.parseInt(parser.getAttributeValue(3));
                    Global.mobDB[i].mob.arm = Integer.parseInt(parser.getAttributeValue(4));
                    Global.mobDB[i].mob.spd = Integer.parseInt(parser.getAttributeValue(5));
                    Global.mobDB[i++].mob.dmg = Integer.parseInt(parser.getAttributeValue(6));
                }
                parser.next();
            }
        }catch(Throwable t){}
