...
class InventoryView(context: Context) : View(context) {
    ...
    private val maxItemsOnScreen = 13
    private var curItemsOnScreen: Int = 0
    private var curScroll: Int = 0
    ...
    private val mItemPanelCombinedHeight: Int // ItemPanelHight + SpaceBetweenItemPanels
    ...
    private val mItemListRegion: Region
    ...
    init {
        ...
        mItemListRegion = Region(mItemPanelsLeftBorder.toInt(), mItemPanelsTopBorder.toInt(), mItemPanelsRightBorder.toInt(), mItemPanelsBottomBorder.toInt())
        ...
        mMaxItemsOnScreen = if (mItemListRegion.bounds.height() % mItemPanelCombinedHeight != 0) {
            mItemListRegion.bounds.height() / mItemPanelCombinedHeight + 1
        } else {
            mItemListRegion.bounds.height() / mItemPanelCombinedHeight
        }
        fillList()
    }
    ...
    fun fillList(weapon: Boolean, shield: Boolean, armor: Boolean, gear: Boolean, consumable: Boolean) {
        ...
        if (temp != null)
            temp.nextList = null
        maxScroll = if (curItemsOnScreen > maxItemsOnScreen) -(curItemsOnScreen - maxItemsOnScreen) * mItemPanelCombinedHeight else 0
    }
    ...
    internal fun drawList(canvas: Canvas) {
        canvas.drawRect(mItemListRegion.bounds, mMainBackgroundPaint)
        var cur = list
        if (cur != null) {
            val u = -(curScroll + savedScroll) / mItemPanelCombinedHeight
            var q = 0
            while (cur != null) {
                if (q >= u) {
                    val top = mItemPanelsTopBorder + q * (mSpaceBetweenItemPanels + mItemPanelHeight)
        ...
        if (mDrawGear) {
            ...
            if (curItem!!.item.isConsumable) {
                ...
                if (Global.hero!!.equipmentList[curItem!!.item.type - 1] == null) {
                    mLeftSoftButton.mLabel = "Надеть"
                } else {
                    if (curItem === Global.hero!!.equipmentList[curItem!!.item.type - 1]) {
                        mLeftSoftButton.mLabel = "Снять"
                    } else {
                        canvas.drawText("�������", 35f, 765f, mSecondaryTextPaint) // I have no idea what that is
                    }
                }
            }
            mSecondaryTextPaint.textAlign = Paint.Align.CENTER
    ...
    fun onTouchInv(sx: Int, sy: Int) {
        ...
        if (mItemListRegion.contains(sx, sy)) {
            if (maxScroll == 0) {
                ...
                //id = (sy - 72 + -savedScroll % mItemPanelCombinedHeight) / mItemPanelCombinedHeight + -savedScroll / mItemPanelCombinedHeight
                id = (sy - mItemPanelsTopBorder.toInt() + -savedScroll % mItemPanelCombinedHeight - savedScroll) / mItemPanelCombinedHeight
                curItem = findItem(id)
                img = Bitmap.createScaledBitmap(curItem!!.item.image, 168, 168, false)
                scrollPermission = false
                mDrawItem = true
